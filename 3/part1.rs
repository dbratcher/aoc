fn visit_points(directions: String) -> Vec<(i32, i32)> {
    let mut visited_points = Vec::new();
    let mut current_point = (0, 0);
    for direction in directions.split(',') {
        let direction_letter = direction.chars().next().unwrap();
        ...
        } else if direction_letter == 'L' {
            for _ in 0..distance {
                current_point = (current_point.0 - 1, current_point.1);
                visited_points.push(current_point);
            }
        } else if direction_letter == 'U' {
            for _ in 0..distance {
                current_point = (current_point.0, current_point.1 + 1);
                visited_points.push(current_point);
            }
        } else if direction_letter == 'D' {
            for _ in 0..distance {
                current_point = (current_point.0, current_point.1 - 1);
                visited_points.push(current_point);
            }
...
fn main() {
    ...
    let wire1_points = visit_points(wire1.to_string());
    println!("wire 1 points {}", wire1_points.len());
    let wire2_points = visit_points(wire2.to_string());
    println!("wire 2 points {}", wire2_points.len());
    let mut intersections = Vec::new();

    for point in wire1_points.iter() {
        if wire2_points.contains(point) {
            intersections.push(point);
            println!("Found new intersection {:?}", point);
        }
    }

    let first_point = intersections.iter().next().unwrap();
    let mut shortest = first_point.0.abs() + first_point.1.abs();
    for point in intersections.iter() {
        let result = point.0.abs() + point.1.abs();
        if result < shortest {
            shortest = result;
            println!("Found new shortest {}", shortest);
        }
    }
